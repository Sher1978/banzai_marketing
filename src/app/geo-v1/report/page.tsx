...
export default async function GeoReportPage(props: PageProps) {
  const params = props.searchParams ? (props.searchParams.then ? await props.searchParams : props.searchParams) : {};
  ...
  return (
    <html lang="en">
      ...
      <body>
              ...
              {website ? (
                <>
                  <div className="meta-item">
                    <div className="meta-label">Audited Site</div>
                    <div className="meta-value" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '200px' }}>
                      {website}
                    </div>
                  </div>
                  <div className="meta-item">
                    <div className="meta-label">GEO Score</div>
                    <div className="meta-value" style={{ color: 'var(--gold-light)' }}>{score}</div>
                  </div>
                  <div className="meta-item">
        ...
        {/* ── SECTION 0: INDIVIDUAL GEO AUDIT (Dynamic) ── */}
        {website && (
          <div className="section" style={{ borderBottom: '1px solid rgba(197,168,128,0.15)', background: 'var(--surface)' }}>
            <div className="section-label" style={{ color: 'var(--gold-light)' }}>00 — Individual GEO Audit Summary</div>
            <h2 className="section-title">Personalized Brand AI Visibility</h2>
            <p className="section-desc">
              Prepared in real-time for <strong style={{ color: 'var(--text)' }}>{website}</strong> inside the industry <strong style={{ color: 'var(--text)' }}>{params.industry || 'General'}</strong> in region <strong style={{ color: 'var(--text)' }}>{region || 'Global'}</strong>.
            </p>

            <div className="stats-row">
              <div className="stat-card" style={{ border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                <div className="stat-num red" style={{ color: 'var(--red)' }}>{score}</div>
                <div className="stat-label">GEO Visibility Index: Aggregated score based on LLM presence, Schema.org completeness, and brand authority.</div>
              </div>
              <div className="stat-card">
                <div className="stat-num gold">{presence}</div>
                <div className="stat-label">Brand Presence Level: Frequency of brand citations across simulated search matrices of major LLMs.</div>
              </div>
              <div className="stat-card">
                <div className="stat-num" style={{ color: 'var(--green)' }}>{accuracy}</div>
                <div className="stat-label">Data Accuracy Metric: Technical crawling readiness, Schema.org entities, and bot access directives.</div>
              </div>
            </div>

            <div className="callout">
              <div className="callout-label">AI Sentiment Assessment: {sentiment}</div>
              <div className="callout-text">
                The overall brand citation sentiment computed during analysis. A neutral or negative sentiment indicates vulnerability to competitor recommendations hijacking.
              </div>
            </div>

            <div className="alert" style={{ background: 'rgba(197, 168, 128, 0.03)', borderColor: 'rgba(197, 168, 128, 0.15)', borderLeft: '3px solid var(--gold)' }}>
              <div className="alert-label">🛠 Priority GEO Actions Required</div>
              <div className="alert-text">
                1. Establish valid JSON-LD schemas (LocalBusiness and ContactPoint entities).<br />
                2. Configure robots.txt to explicitly allow AI agents (GPTBot, ClaudeBot, PerplexityBot).<br />
                3. Seed structured Brand Ground Truth data across high-authority indexes.
              </div>
            </div>
          </div>
        )}
