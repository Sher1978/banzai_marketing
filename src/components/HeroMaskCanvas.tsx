...
export const HeroMaskCanvas: React.FC<HeroMaskCanvasProps> = ({ targetRef }) => {
    ...
    const targetFrame = useTransform(scrollYProgress, [0, 1], [START_FRAME, END_FRAME]);
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            if (currentImage && currentImage.complete) {
                ...
                // Logic for "Cover" - ensure no empty space on mobile/desktop
    ...
    return (
        <div className="w-full h-full relative flex flex-col items-center justify-center overflow-hidden">
            ...
            {currentFrame >= 31 && (
                ...
                >
                    <span className="text-[10px] md:text-sm hud-text font-bold">NEURAL LINK: ACTIVE</span>
                    <span className="text-[10px] md:text-sm hud-text opacity-70">SYSTEM STATUS: OPTIMAL</span>
                </motion.div>
            )}
            ...
            {currentFrame >= 31 && (
                ...
                >
                    <span className="text-[10px] md:text-sm hud-text font-bold">+300% REACH_ACCELERATION</span>
                    <span className="text-[10px] md:text-sm hud-text opacity-70">+130% SALES_GROWTH</span>
                </motion.div>
            )}
